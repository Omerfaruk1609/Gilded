...
```
                               ...
                               │  - Web Audio API (432Hz Zen Engine)     │
                               │  - Socket.io Client (Real-Time Sub)     │
                               └────────────────────┬────────────────────┘
                                                    │ HTTP / WSS
                                                    ▼
                               ┌─────────────────────────────────────────┐
                               ...
                               │  - Akıllı Rate Limiter (DoS Kalkanı)    │
                               │  - JWT Kimlik ve Rol Doğrulama          │
                               │  - Socket.io Engine (Canlı Çemberler)   │
                               └───────────┬─────────────────┬───────────┘
                                           │                 │
                      ┌────────────────────┘                 └────────────────────┐
                      ▼                                                           ▼
       ┌───────────────────────────────┐                           ┌──────────────────────────────┐
       │   KintsugiText Moderasyon    │                           │    PostgreSQL Veritabanı     │
       │ - Mikro-Saniye Kural Motoru   │                           │ - Cascade İlişkiler (GDPR)   │
       │ - Türkçe Küfür/Toksisite      │                           │ - İndekslenmiş Şema Tabloları│
       │ - Gemini AI Felsefi Tavsiye   │                           │ - Supabase / Neon / Local DB │
       └───────────────────────────────┘                           └──────────────────────────────┘
```
...
```bash
Gilded/
├── server/                     # Backend API & WebSocket Sunucusu
│   ├── index.cjs               # Express sunucusu, rotalar, rate limiter ve socket olayları
│   ├── db.cjs                  # PostgreSQL havuz bağlantısı ve şema ilklendirme
│   ├── schema.sql              # PostgreSQL DDL tablo şemaları ve indeksler
│   ├── uploads/                # Yüklenen görseller ve ses dosyaları (Statik sunulur)
│   └── utils/
│       ├── kintsugiModeration.cjs  # KintsugiText küfür ve toksisite filtresi
│       └── philosopherBot.cjs      # Felsefi AI bot ve token optimizasyonu
├── src/                        # React Frontend (Vite)
│   ├── components/
│   │   ├── feed/               # PostCard, PostForm, PostModal, AudioPlayer
│   │   ├── home/               # HeroSection, MetricsSection, QuotesSection
│   │   ├── layout/             # Navbar, Footer, NotificationsMenu, MeditationModal, FloatingQuickAction
│   │   └── guards/             # ProtectedRoute, AdminRoute, PublicOnlyRoute
│   ├── context/                # ThemeContext, SocketContext
│   ├── pages/                  # HomePage, CirclesPage, WisdomPage, MessagesPage, ProfilePage, HallOfFame...
│   ├── services/               # apiClient, apiConfig, auth
│   └── utils/                  # ambientAudio (Web Audio API Motoru), soundEffects
...
### 1. Depoyu Klonlayın
```bash
git clone https://github.com/Omerfaruk1609/Gilded.git
cd Gilded
```
